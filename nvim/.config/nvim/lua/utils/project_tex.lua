...
local contents = require("utils.variables")
...
local function create_floating_window()
  ...
  vim.fn.prompt_setprompt(buf, "Nombre del ensayo: ")
...
local function setup_project(project_name)
  ...
  if project_name and project_name ~= "" then
    local project_dir = vim.fn.expand "$HOME" .. "/Documentos/Ensayos/" .. project_name
    ...
    vim.fn.writefile(vim.split(contents.tex_content, "\n"), main_tex_path)

    vim.cmd("edit " .. main_tex_path)

    local justfile_path = project_dir .. "/.justfile"
    local justfile_content = contents.justfile_content(project_name)
